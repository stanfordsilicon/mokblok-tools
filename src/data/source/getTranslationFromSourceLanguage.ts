...
function getTranslationFromSourceLanguage({
  ...
  getInnerString,
  sourceLanguage,
}: getTranslationFromSourceLanguageParams): { translation: string; pattern?: string } {
  const sourceData = getSourceData(entry);

  if (sourceData) {
    if (!entry.exampleNum || entry.exampleNum === '0') {
      // It's a direct translation
      if (sourceLanguage === 'en-Latf') return { translation: getFraktur(sourceData) };
      return { translation: sourceData };
    } else {
      ...
      if (entry.patternFormat === PatternFormat.Substitution) {
        ...
        return { translation: value, pattern: sourcePattern };
  ...
  if (sourceLanguage === 'fr' && entry.french) {
    return { translation: entry.french };
  } else if (sourceLanguage === 'en-Latf') {
    return { translation: getFraktur(entry.english) };
  }
  return { translation: entry.english };
}
